"""
WhatsApp channel for value_claw (via the WhatsApp Cloud API).
...
* A publicly reachable HTTPS callback URL pointing to ``/whatsapp/webhook``
  on the value_claw web server (use ngrok for local development).
...
Access control
--------------
Set ``channels.whatsapp.allowedNumbers`` in ``value_claw.json`` to a list of
...
def create_bot(session_manager: "SessionManager") -> WhatsAppBot:
    """Create a WhatsAppBot from value_claw.json / env vars."""
