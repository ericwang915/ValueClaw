#!/usr/bin/env python3
"""Spotify Web API controller for value_claw."""
import argparse
import json
import os
import sys
...
TOKEN_FILE = os.path.expanduser("~/.value_claw/.spotify_token.json")
REDIRECT_URI = "http://localhost:8888/callback"
SCOPES = "user-read-playback-state user-modify-playback-state user-read-currently-playing"


def _get_config() -> dict:
    for path in [os.path.expanduser("~/.value_claw/value_claw.json"), "value_claw.json"]:
...
def cmd_auth(args):
    cfg = _get_config()
    if not cfg.get("clientId"):
        print("Set skills.spotify.clientId and clientSecret in value_claw.json first")
        return
