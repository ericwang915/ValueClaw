---
name: change_setting
description: "Modify value_claw.json configuration at runtime. Use when: user wants to set API keys, tokens, change LLM provider, adjust web port, or update any config value. NOT for: editing code, changing soul/persona files, or modifying skill files."
metadata:
  emoji: "⚙️"
---
...
Modify `value_claw.json` configuration at runtime.
...
- Uses bundled `update_config.py` to read and update value_claw.json
- API keys, passwords, and tokens are masked when displaying config
- Only `value_claw.json` is modified — no other files
