#!/usr/bin/env python3
"""Read and update value_claw.json configuration values."""
...
def _config_file() -> str:
    home = os.path.expanduser("~/.value_claw")
    for p in [os.path.join(home, "value_claw.json"), "value_claw.json"]:
        if os.path.exists(p):
            return p
    return os.path.join(home, "value_claw.json")
...
def main():
    parser = argparse.ArgumentParser(description="Manage value_claw.json")
