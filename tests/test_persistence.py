...
def make_minimal_agent(tmp_path, provider=None, store=None, session_id="test:1", cron_manager=None):
    ...
    from value_claw.core.persistent_agent import PersistentAgent
    from value_claw.core.agent import Agent
...
class TestSessionStore:
    def test_save_and_load_roundtrip(self, tmp_path):
        from value_claw.core.session_store import SessionStore
...
class TestAgentCronTools:
    """Tests for PrefectScheduler dynamic job management (no event loop needed)."""

    def _make_cron_manager(self, tmp_path):
        """Return a PrefectScheduler with a mock scheduler (no event loop required)."""
        ...
        with open(jobs_path, "w") as f:
            f.write("jobs: []\n")
        c = PrefectScheduler(session_manager=sm, jobs_path=jobs_path)
    ...
    def test_cron_add_persisted_to_json(self, tmp_path):
        from value_claw.scheduler.prefect_scheduler import _dynamic_jobs_file as _djf; DYNAMIC_JOBS_FILE = _djf()
        cron_mgr = self._make_cron_manager(tmp_path)
