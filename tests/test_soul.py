...
from value_claw.core.agent import Agent, _load_text_dir_or_file
...
class TestSessionManager:
    def _make_sm(self):
        from value_claw.session_manager import SessionManager
        calls = []
        def factory(sid):
            calls.append(1)
            return MagicMock()
...
class TestTelegramBot:
    ...
    def test_import(self):
        from value_claw.channels.telegram_bot import TelegramBot, create_bot_from_env
    ...
    def test_split_message_short(self):
        from value_claw.channels.telegram_bot import _split_message
...
class TestCronScheduler:
    ...
    def test_import(self):
        from value_claw.scheduler.prefect_scheduler import PrefectScheduler, _parse_cron
        assert PrefectScheduler is not None

    def test_parse_cron_valid(self):
        from value_claw.scheduler.prefect_scheduler import _parse_cron
    ...
    def test_load_jobs_missing_file(self, tmp_path):
        from value_claw.scheduler.prefect_scheduler import PrefectScheduler
        s = PrefectScheduler(session_manager=self._make_sm(), jobs_path=str(tmp_path / "missing.yaml"))
        jobs = s._load_jobs()
        assert jobs == []
    ...
    def test_load_jobs_from_yaml(self, tmp_path):
        ...
        s = PrefectScheduler(session_manager=self._make_sm(), jobs_path=str(jobs_file))
        jobs = s._load_jobs()
        assert len(jobs) == 1
    ...
    def test_each_job_gets_isolated_session(self, tmp_path):
        ...
        s = PrefectScheduler(session_manager=sm, jobs_path=str(jobs_file))
...
class TestHeartbeatMonitor:
    def test_import(self):
        from value_claw.scheduler.heartbeat import HeartbeatMonitor, create_heartbeat_from_env
        assert HeartbeatMonitor is not None

    def test_create_from_env_defaults(self, monkeypatch):
        from value_claw.scheduler.heartbeat import create_heartbeat_from_env, DEFAULT_INTERVAL
    ...
    def test_create_from_env_custom(self, monkeypatch):
        from value_claw.scheduler.heartbeat import create_heartbeat_from_env
    ...
    def test_probe_ok_sets_last_ok_true(self):
        import asyncio
        from value_claw.scheduler.heartbeat import HeartbeatMonitor
        provider = MagicMock()
        provider.chat.return_value = MagicMock(
